<?php session_start(); 
	...
	$sql = "SELECT * FROM tbl_regusers WHERE email='$user_name' AND password='".PasswordEncryption($password)."' AND status = 'active' LIMIT 1";
	...
	if(mysqli_num_rows($reslogin)>0){	    
		...
		if(mysqli_query($con,$updatelogin)){				 
			header('Location:student/myprofile.php?lmsg=Welcome to login successfully here.');						
		}else{
			header('Location:login.php?lmsg=Email Id or password wrong.');
		}    
  	} else{
		header('Location:login.php?lmsg=Email Id or password wrong.');
	} 
