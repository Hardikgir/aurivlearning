<?php session_start(); 
	...
	$sql = "SELECT * FROM tbl_regusers WHERE email='$user_name' AND password='".PasswordEncryption($password)."' AND status = 'active' AND utype = '0' LIMIT 1";
	...
	if(mysqli_num_rows($reslogin)>0){
		...
		if(mysqli_query($con,$updatelogin)){				 
			header('Location:dashboard.php?lmsg=Welcome to login successfully here.');						
		}else{
